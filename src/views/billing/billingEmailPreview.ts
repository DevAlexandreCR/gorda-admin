...
export function buildBillingEmailPreviewHtml({
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #e6eaf2;border-radius:18px;overflow:hidden;box-shadow:0 16px 36px rgba(15,23,42,0.08);color:#344767 !important;">
    ...
    .map(
      (header, index) =>
        `<th style="padding:12px 16px;background:#f8fafc;color:#7b809a;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;text-align:${
          index === 0 ? "left" : "right"
        };border-bottom:1px solid #e9edf5;color:#7b809a !important;">${escapeHtml(
          header
        )}</th>`
    )
    .join("");
