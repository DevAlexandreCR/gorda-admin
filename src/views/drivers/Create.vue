<template>
  <div class="container-fluid pb-4" v-if="driver.id">
          ...
          <div class="row">
            <div class="col-md-5">
              <div class="card-header p-0 mx-3 mt-3 z-index-1">
                <img :src="this.driver.photoUrl" class="img-fluid border-radius-lg" alt="profile_photo">
            ...
            <div class="col-md-7">
              <div class="form-group">
                ...
                <Field name="name" type="text" class="form-control" :placeholder=" $t('common.placeholders.name')" v-model="driver.name" id="name" aria-label="Name" aria-describedby="name-addon" />
                <ErrorMessage name="name"/>
              </div>
              <div class="form-group">
                <label>{{ $t('users.fields.email') }}</label>
                <Field name="email" type="email" class="form-control" id="email" :placeholder="$t('common.placeholders.email')" v-model="driver.email" aria-label="Email" aria-describedby="email-addon" />
                <ErrorMessage name="email"/>
              </div>
              <div class="form-group">
                <label>{{ $t('users.fields.phone') }}</label>
                <Field name="phone" type="phone" class="form-control" :placeholder=" $t('common.placeholders.phone')" id="phone" v-model="driver.phone" aria-label="Phone" aria-describedby="phone-addon"/>
                <ErrorMessage name="phone"/>
              </div>
              <div class="form-group">
                <label>{{ $t('users.fields.role') }}:</label>
                <div class="form-check mb-4 d-inline-block ms-4">
                  <input class="form-check-input" type="checkbox" @change="assignRole" name="role" id="operator" :value="'operator'" :checked="driver.roles.operator"/>
                  <label class="custom-control-label">{{ $t('users.fields.operator') }}</label>
                </div>
                <div class="form-check d-inline-block ms-3">
                  <input class="form-check-input" type="checkbox" name="role" @change="assignRole" id="admin" :value="'admin'" :checked="driver.roles.admin"/>
                  <label class="custom-control-label">{{ $t('users.fields.admin') }}</label>
                </div>
                <ErrorMessage name="role"/>
              </div>
              <div class="form-check form-switch">
                <input class="form-check-input" name="enable" type="checkbox" id="flexSwitchCheckDefault" :checked="driver.isEnabled()" @change="onEnable"/>
...
export default class Edit extends Vue {
  ...
  uploadImg(): void {
    const ref = StorageService.stAdminProfileImages(this.driver.id?? '', this.image[0]?.name)
  ...
  createDriver(): void {
    UserRepository.update(this.driver).then(() => {
      Swal.fire({
        icon: 'success',
  ...
  assignRole(e: Event): void {
    const target = e.target as HTMLInputElement
    if (this.driver.roles) {
      if (target.value === 'operator') {
        this.driver.roles.operator = target.checked
      } else {
        this.driver.roles.admin = target.checked
      }
    }
  }
  ...
  created(): void {
    UserRepository.getUser(this.$route.params.id as string).then(user => {
      Object.assign(this.driver, user)
      this.driver.photoUrl = this.driver.photoUrl?? '../../assets/img/logo.png'
    })
  }
}
</script>